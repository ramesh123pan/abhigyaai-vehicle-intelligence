...
| PLAN-02 | Top-up packages full CRUD | In Plan Management, create a package with name, whole-number calls, INR price, and Active status; edit all four fields including switching Active to Inactive; deactivate it; try blank name, zero/decimal calls, and negative price. Verify the API payload and refreshed table after each action. | Create returns 201; edit persists name/calls/price/status; deactivate persists inactive status without deleting billing history; invalid values return 400; duplicate names return a clear error. |
...
| 2026-09-25 | Chrome + local workspace | Browser create/edit flow; `npm test` | Chrome created and edited a package with all fields; automated full top-up CRUD assertions are present but skipped when test credentials are unavailable. | Yes for top-up create/edit | Server process verified listening on 4173 | Browser QA package remains for cleanup; delete/deactivation was not browser-click verified. |
