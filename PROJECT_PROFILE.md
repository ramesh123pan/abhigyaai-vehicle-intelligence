...
The API Documentation surface now uses the full available content width instead of the legacy 1,240px cap. Test results are now integrated into the shared application shell at `#test-status`, with automated counts, evidence rows, load-test results, feature verification, and skipped-test prerequisites. The standalone `/test-status.html` remains as a fallback, while API Documentation links to the shared in-app QA page.
